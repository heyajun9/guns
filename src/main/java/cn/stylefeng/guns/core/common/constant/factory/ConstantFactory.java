...
import cn.stylefeng.guns.modular.servlet.entity.*;
import cn.stylefeng.guns.modular.servlet.mapper.*;
...
public class ConstantFactory implements IConstantFactory {
    ...
    private InterMapper interMapper = SpringContextHolder.getBean(InterMapper.class);
    private WmsItemMapper wmsItemMapper = SpringContextHolder.getBean(WmsItemMapper.class);
    private WmsCountDownMapper wmsCountDownMapper = SpringContextHolder.getBean(WmsCountDownMapper.class);
    private WmsAsnOrderMapper wmsAsnOrderMapper = SpringContextHolder.getBean(WmsAsnOrderMapper.class);
    private QuartzTaskMapper quartzTaskMapper = SpringContextHolder.getBean(QuartzTaskMapper.class);
    private WmsPickTicketOrderMapper wmsPickTicketOrderMapper = SpringContextHolder.getBean(WmsPickTicketOrderMapper.class);
    private WmsOrganizationMapper organizationMapper = SpringContextHolder.getBean(WmsOrganizationMapper.class);
    private TableMapper tableMapper = SpringContextHolder.getBean(TableMapper.class);
    ...
    public String getJobName(Long jobId) {
        if (ToolUtil.isEmpty(jobId)) {
            ...
            QuartzTask task = quartzTaskMapper.selectById(jobId);
            if (task == null) {
                return "";
            } else {
    ...
    @Override
    public String getInterfaceName(Long interId) {
        if (ToolUtil.isEmpty(interId)) {
            ...
            if (inter == null) {
                ...
                return inter.getInterfaceName();
            }
        }
    }
    @Override
    public String getItemCode(Long liusNo) {
        if (ToolUtil.isEmpty(liusNo)) {
            return "";
        } else {
            WmsItem wmsItem = wmsItemMapper.selectById(liusNo);
            if (wmsItem == null) {
                return "";
            } else {
                return wmsItem.getShangp_id();
            }
        }
    }

    @Override
    public String getOrderCode(Long asnId) {
        if (ToolUtil.isEmpty(asnId)) {
            return "";
        } else {
            WmsAsnOrder wmsAsnOrder = wmsAsnOrderMapper.selectById(asnId);
            if (wmsAsnOrder == null) {
                return "";
            } else {
                return wmsAsnOrder.getDingd_no();
            }
        }
    }
    ...
    @Override
    public String getOrderCodeP(Long picticket) {
        if (ToolUtil.isEmpty(picticket)) {
            return "";
        } else {
            WmsPickTicketOrder wmsPickTicketOrder = wmsPickTicketOrderMapper.selectById(picticket);
            if (wmsPickTicketOrder == null) {
                return "";
            } else {
                return wmsPickTicketOrder.getDanj_no();
            }
        }
    }
    ...
    @Override
    public String getOrganizationCode(Long liusNo) {
        if (ToolUtil.isEmpty(liusNo)) {
            return "";
        } else {
            WmsOrganization organization =organizationMapper.selectById(liusNo);
            if (organization == null) {
                return "";
            } else {
                return organization.getDanw_id();
            }
        }
    }
