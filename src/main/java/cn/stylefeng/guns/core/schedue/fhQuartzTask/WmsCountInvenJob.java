...
public class WmsCountInvenJob implements Job {
			...
			for(Inter inter:inters){
				...
				List<TableOption> tableOptions=tableService.selectTableOption(interId);
						...
						if(countInventories!=null&&countInventories.size()>0){
							...
							WmsCountInventory wmsCountInventory=countInventories.get(0);
									...
									if("bodys".equals(key)){
										for(WmsCountInventory countInventory:countInventories){
											...
											for(String str:strings) {
												...
												String up=strings1[0];
												String down=strings1[1];
												Object obj = wmsCountInventoryService.selectObject(down, countInventory.getDanj_no(), String.valueOf(countInventory.getHanghao()));
												object.put(up,obj);
											}
											array.add(object);
										}
										...
										Object obj = wmsCountInventoryService.selectObject(value, wmsCountInventory.getDanj_no(), String.valueOf(wmsCountInventory.getHanghao()));
										jsonput.put(key, obj);
									}
								}
//								arr.add(jsonput);
//							}
							...
							json.put("orgcode",wmsCountInventory.getHuoz_id()=="1001"?"217":"020");//217代表股份，020代表电商
							...
							msg+="request:"+json;
							//需要设置用户名和密码
							String ret=newClient.httpClientPostForm(inter.getUrl().trim(), json, inter.getUserName().trim(), inter.getPassword().trim());
								...
								if("0".equals(state)){
									if(countInventories!=null&&countInventories.size()>0){
										for(WmsCountInventory countInventory:countInventories){
											...
											wmsCountInventoryService.updateWmsCountInventory(countInventory);
										}
									}
								}else{
									if(countInventories!=null&&countInventories.size()>0){
